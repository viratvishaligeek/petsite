...
@section('content')
    ...
    <form class="row mb-3" action="{{ route('admin.blog.update', encrypt($data->id)) }}" method="post">
        @csrf
        @method('PATCH')
        <div class="col-xl-9">
            <div class="row g-3">
                <div class="col-12">
                    <div class="form-floating">
                        <input class="form-control" name="name" required value="{{ old('name') ?? $data->name }}"
                ...
                <div class="col-12">
                    <div class="form-floating">
                        <input type="date" class="form-control" name="publish_date" required
                            value="{{ old('publish_date') ?? ($data->publish_date ?? date('Y-m-d')) }}" />
                        ...
                        <select class="form-select" name="publisher_id" required>
                            ...
                            @foreach ($publishers as $publisher)
                                <option value="{{ $publisher->id }}"
                                    {{ old('publisher_id') ?? $data->publisher_id == $publisher->id ? 'selected' : '' }}>
                                    {{ $publisher->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="col-12">
                    <div class="form-floating">
                        <select class="form-select" name="status" required>
                            ...
                            <option value="draft" {{ old('status') ?? $data->status == 'draft' ? 'selected' : '' }}>Draft
                            </option>
                            <option value="published"
                                {{ old('status') ?? $data->status == 'published' ? 'selected' : '' }}>Published
                            </option>
                            <option value="archived" {{ old('status') ?? $data->status == 'archived' ? 'selected' : '' }}>
                                Archived</option>
                        </select>
                        <label>Status</label>
                    </div>
...
@section('script')
    ...
    <script>
        $(document).ready(function() {
            let oldTags = "{{ old('tags') ?? $data->tags }}";
